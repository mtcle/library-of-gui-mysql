        ...
        try {
          ...
          }
          {
            ...
            statement.execute("update record set checkin='" + timeformat.format(tempdate.getTime())
                + "',rent='" + df.format(jisuanzujin) + "',fajin='" + df.format(jisuanfajin) + "'where bookid='"
                + getbookid + "'");
            Statement statement2 = connection.createStatement();
            statement2.execute("update book set status=0 where bookid='" + getbookid + "'");
            connection.close();
            JOptionPane.showMessageDialog(null, "租金：" + df.format(jisuanzujin) + "元 罚金：" + df.format(jisuanfajin)
                + "元 总计：" + df.format(jisuanzujin + jisuanfajin) + "元", "提示", 1);
        ...
        try {
         ...
         if(sum>0){
          if (getplan - getdate > (21 * 24 * 60 * 60 * 1000l)) {
            ...
            Statement statement = connection.createStatement();
            statement.execute("update record set plan='" + newtime + "' where bookid='" + getbookid + "'");// 计算后的日期赋值给他
